...
// v4.3: online feature model (wall time regression)
let featureModel = null;
if (db) {
  try { featureModel = await getFeatureModel(db, "wall_ms_linear_v1"); } catch (_) { featureModel = null; }
}

// v4.4: plan family cache (enumeration caching)
let familyCache = null;
if (db) {
  try { familyCache = await getPlanFamilyCache(db, inputDagHash); } catch (_) { familyCache = null; }
}
...
// v4.4: cache hit => skip enumeration
if (familyCache && familyCache.bestPlan) {
  usedCache = true;
  opt = {
    ok: true,
    before: compiled0.plan,
    after: familyCache.bestPlan,
    changes: familyCache.changes || [],
    cost: familyCache.bestCost || null,
    variants: { candidates: familyCache.candidates || [] }
  };
} else {
  opt = optimizeDagV3_8(compiled0.plan, statsByNodeId, joinSigStats, predSigStats, predGroupStats, joinFilterStats);
}

// Build candidate hashes so we can fetch plan_variant_stats for bandit selection
...
// v4.4: pass optOverride so optimizer loop doesn't re-enumerate
const sel = optimizeAndSelectV4(compiled0.plan, {
  statsByNodeId, joinSigStats,
  predSigStats, predGroupStats, joinFilterStats,
  planVariantStats,
  featureModel,
  inputDagHash
}, { optOverride: opt });
...
const compiled = compileDagSkeleton(execPlan, { assignIdsIfMissing: false });
  ...
  // v4.6: attempt streaming fast-path for linear pipelines
  let streamedExec = null;
  if (db && process.env.AXIOMFLOW_STREAMING !== '0') {
    try { streamedExec = await tryExecuteStreamingPipeline(compiled.plan, { db, runId }); } catch (_) { streamedExec = null; }
  }
...
// v4.4: cache enumerated candidates by plan family
if (db && opt?.ok && (!usedCache || process.env.AXIOMFLOW_CACHE_REFRESH === '1')) {
  try {
    const candidates = (opt.variants?.candidates || []).map((c)=>({
      rank: c.rank,
      plan: c.plan,
      cost: c.cost || null,
      estCost: c.cost?.summary?.totalCost ?? c.estCost ?? null,
      chosenOrder: c.chosenOrder || null
    }));
    await upsertPlanFamilyCache(db, inputDagHash, {
      bestPlan: opt.after,
      bestCost: opt.cost || null,
      candidates,
      changes: opt.changes || []
    });
  } catch (_) {}
  ...
  let lineage = new LineageDagCollector({ runId, plan: compiled.plan });
  let materialized = new Map();
  if (streamedExec && streamedExec.ok) {
    lineage = streamedExec.lineage;
    materialized = streamedExec.materialized;
  }
...
try {
  await writeOptimizerDecision(db, {
    ...
    candidates: execMeta?.candidates ?? null,
    chosenRank: execMeta?.chosenRank ?? null,
    chosenCost: execMeta?.chosenCost ?? null
  });
} catch (_) {}
...
// v4.3: update wall-time feature model
try {
  if (db) {
    const x = extractFeatures(compiled.plan, opt.cost);
    const wallMs = Date.now() - t0Wall;
    const nextModel = updateModelSgd(featureModel || {}, x, wallMs, {
      lr: parseFloat(process.env.AXIOMFLOW_MODEL_LR || "0.001"),
      l2: parseFloat(process.env.AXIOMFLOW_MODEL_L2 || "0.0001")
    });
    await upsertFeatureModel(db, "wall_ms_linear_v1", nextModel);
  }
} catch (_) {}
  }
