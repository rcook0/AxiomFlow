...
async function updatePlanVariantStats(db, familyDagHash, variantDagHash, wallMs, costSummary, opts = {}) {
  ...
  const wallMs2 = (wallMs == null) ? null : (wallMs * wallMs);

  const emaWall = ema(existing?.emaWallMs, wallMs, alpha);
  const emaWall2 = ema(existing?.emaWallMs2, wallMs2, alpha);
  const emaVar = (emaWall != null && emaWall2 != null) ? Math.max(0, emaWall2 - (emaWall * emaWall)) : existing?.emaVar;
...
async function getFeatureModel(db, name) {
  const col = db.collection("runtime_models");
  const doc = await col.findOne({ name });
  return doc || null;
}

async function upsertFeatureModel(db, name, model) {
  const col = db.collection("runtime_models");
  const next = {
    schemaVersion: 1,
    name,
  ...
  await col.updateOne({ name }, { $set: next }, { upsert: true });
  return next;
}

module.exports.getFeatureModel = getFeatureModel;
module.exports.upsertFeatureModel = upsertFeatureModel;

async function getPlanFamilyCache(db, familyDagHash) {
  const col = db.collection("plan_family_cache");
  const doc = await col.findOne({ familyDagHash });
  return doc || null;
}

async function upsertPlanFamilyCache(db, familyDagHash, payload) {
  const col = db.collection("plan_family_cache");
  const next = {
    schemaVersion: 1,
    familyDagHash,
  ...
  await col.updateOne({ familyDagHash }, { $set: next }, { upsert: true });
  return next;
}

module.exports.getPlanFamilyCache = getPlanFamilyCache;
module.exports.upsertPlanFamilyCache = upsertPlanFamilyCache;
