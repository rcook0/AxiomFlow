...
## V4.x optimizer loop enhancements

- **Selection policies** live in `backend/optimizer/*` (UCB / Thompson / epsilon / best).
- **Plan family cache** (`plan_family_cache`) stores enumerated candidate sets keyed by `familyDagHash`.
- **Runtime model** (`runtime_models`) stores `wall_ms_linear_v1` online regression weights for cold-start estimates.
- **Streaming fast-path** (`backend/engine/executePipelineStreaming.js`) executes linear pipelines via cursor iteration.
