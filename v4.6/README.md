...
- Learns selectivity for AND predicate groups (`pred_group_stats`) and join→filter segments (`join_filter_stats`).
- Supports epsilon-greedy exploration of alternative join orders when a single join component exists.
- Tracks plan variant performance (`plan_variant_stats`).

See `docs/COMPOSITION_SELECTIVITY.md` and `docs/EXPLORATION.md`.

## v3.9 → v4.0
- Bandit-ready plan selection (UCB over `plan_variant_stats`).
- Persist per-run optimizer decisions (`optimizer_decisions`).
- Introduces `backend/optimizer/*` for selection policy modularity.

See: `docs/V4_OPTIMIZER_LOOP.md`.
...
## v4.1
- Multi-component (multi-root) exploration via coordinate variants.
- Offline replayer: `node scripts/replay-optimizer-decisions.js <mongo> <db> --limit 50`.
See `docs/V4_1_MULTI_COMPONENT.md`.


## V4.2–V4.6 additions

- **Thompson sampling** enabled via variance tracking in `plan_variant_stats`.
- **Online wall-time model** persisted to `runtime_models` (`wall_ms_linear_v1`).
- **Plan family cache** in `plan_family_cache` to skip enumeration for repeated DAGs.
- **Policy lab**: `node scripts/policy-lab.js <mongoUri> <dbName> --family <dagHash>`.
- **Streaming fast-path** for `scan -> (filter|project)* -> sink` pipelines (disable via `AXIOMFLOW_STREAMING=0`).

See `docs/RELEASE_NOTES_v4_6.md` and `docs/V4_POLICY_KNOBS.md`.
