...
function buildProjectMap(projectOp, opts = {}) {
  ...
  if (!projectOp || projectOp.op !== "project" || !projectOp.params || !projectOp.params.exprs || typeof projectOp.params.exprs !== "object") {
    out.ok = false;
    out.issues.push({ field: "*", reason: "Invalid project node (missing params.exprs)" });
    return out;
  }

  for (const [field, expr] of Object.entries(projectOp.params.exprs)) {
    if (isColExpr(expr)) {
      out.map[field] = expr;
