...
function pushTopK(arr, cand, K){
  arr.push(cand);
  arr.sort((a,b)=>a.estCost - b.estCost);
  if (arr.length > K) arr.length = K;
}

/**
 * Bushy DP with top-K retention.
 */
function dpJoinOrderBushyTopK(allRels, baseRelSizes, joinPredicates, joinSigStats, opts){
  ...
  if (n > maxRels) return { ok:false, reason:`too_many_rels(${n})`, maxRels };

  const best = new Map(); // subsetKey -> [states...]
  ...
  for (let sz = 2; sz <= n; sz++) {
    for (let mask = 0; mask < (1<<n); mask++) {
      ...
      for (let aMask = (mask - 1) & mask; aMask > 0; aMask = (aMask - 1) & mask) {
        ...
        const aList = best.get(Akey);
        const bList = best.get(Bkey);
        ...
        for (const aState of aList) {
          for (const bState of bList) {
            ...
            pushTopK(topStates, cand, K);
          }
        }
      }

      if (topStates.length) best.set(Skey, topStates);
    }
  }

  const finals = best.get(subsetKey(allRels));
  if (!finals || finals.length === 0) return { ok:false, reason:"no_feasible_plan" };
  return { ok:true, states: finals };
}
...
/**
 * Materialize a DP state into a join tree plan rewrite.
 */
function materializeStateToPlan(plan, rootJoinId, joins, relsByNode, state, baseRelSizes){
  const { nodeById, out } = buildMaps(plan);
  ...
  function materialize(s){
    ...
    const leftId = materialize(s.left);
    const rightId = materialize(s.right);

    const joinId = mkId("opt8_j", subsetKey(s.rels));
  ...
  const newRootJoinId = materialize(state);
  ...
  // Rewire all consumers of old root to new root
  plan2.edges = plan2.edges.filter(e=>!(e.from===rootJoinId && rootOut.some(ro=>ro.to===e.to)));
  for (const e of rootOut) plan2.edges.push({ from: newRootJoinId, to: e.to, port: e.port || "in" });

  return { ok:true, plan: plan2, afterRootJoinId: newRootJoinId };
}

/**
 * Rewrite join component rooted at rootJoinId into best + top-K alternative plans.
 */
function rewriteJoinTreeV3_8(plan, rootJoinId, baseRelSizes, joinSigStats, opts){
  ...
  const dp = dpJoinOrderBushyTopK(allRels, baseRelSizes || new Map(), allPreds, joinSigStats || new Map(), opts || {});
  if (!dp.ok) return { ok:false, reason:"dp_failed", detail: dp, rootJoinId };

  const states = dp.states;
  const candidates = [];
  for (let i=0;i<states.length;i++) {
    // deep clone state graph by JSON is not safe due to Sets/refs; instead we accept that
    // materialization mutates `rootNodeId` and `bindings` fields, so we need to copy those fields away.
    // We'll rebuild a minimal copy of the state tree.
    function cloneState(s){
      if (s.rels && s.rels.size === 1) {
        return { rels: new Set([...s.rels]), estRows: s.estRows, estCost: s.estCost };
      }
      return {
        rels: new Set([...s.rels]),
        estRows: s.estRows,
        ...
        algo: s.algo,
        build: s.build,
        joinPreds: s.joinPreds,
        joinSig: s.joinSig,
        left: cloneState(s.left),
        right: cloneState(s.right)
      };
    }
    const st = cloneState(states[i]);

    const mat = materializeStateToPlan(plan, rootJoinId, joins, relsByNode, st, baseRelSizes || new Map());
    if (!mat.ok) continue;

    candidates.push({
      rank: i,
      beforeRootJoinId: rootJoinId,
      afterRootJoinId: mat.afterRootJoinId,
      rels: [...allRels],
      estCost: st.estCost,
      estRows: st.estRows,
      chosenOrder: describeState(st),
      plan: mat.plan
    });
  }

  if (candidates.length === 0) return { ok:false, reason:"no_materialized_candidates", rootJoinId };

  return {
    ok:true,
    best: candidates[0],
    candidates,
    dp: { maxRelsForBushy: opts?.maxRelsForBushy ?? 7, topK: opts?.topK ?? 3 }
  };
}
