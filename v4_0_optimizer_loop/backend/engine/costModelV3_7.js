...
function estimatePlanCostV3_7(plan, statsByNodeId, joinSigStats, predSigStats, opts){
  ...
  for (const id of topo) {
    ...
    if (n.op === "filter") {
      ...
      const learnedSel = sig ? (ps.get(sig)?.emaSelectivity ?? null) : null;

      const sel =
        learnedSel ??
        (s.get(id)?.emaSelectivity ?? null) ??
        (n.params?.estimatedSelectivity ?? null) ??
        (o.defaultFilterSelectivity ?? 0.3);
