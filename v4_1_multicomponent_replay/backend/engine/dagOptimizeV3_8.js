...
// Multiple join roots: rewrite each to best in a stable order, but also emit per-component exploration candidates (v4.1).
let multiRootInfo = [];
for (const r of roots) {
  ...
  cur = res.best.plan;
  changes.push({ rule: "JOIN_ORDER_TOPK_DP", nodeId: r.id, detail: { dp: res.dp, best: { estCost: res.best.estCost, estRows: res.best.estRows, chosenOrder: res.best.chosenOrder } } });
  multiRootInfo.push({ rootJoinId: r.id, dp: res.dp });
}

// v4.1: after best plan is built, generate "coordinate" variants:
// for each join component, produce alternatives while keeping other components fixed.
// This avoids cartesian explosion but still explores multi-root DAGs over time.
try {
  const maxTotal = (o.joinOrder?.maxTotalCandidates ?? 12);
  const perRoot = (o.joinOrder?.topK ?? 3);

  variants = { candidates: [], components: [] };
  const baseBest = cur;
  const costBase = estimatePlanCostV3_8(baseBest, s, js, ps, pg, jf, o.costModel || {});
  const rowsBase = costBase.rowsByNode;

  // recompute roots on the best plan (ids may differ post-rewrite)
  const rootsBest = findRootJoins(baseBest);

  for (const rb of rootsBest) {
    // base sizes for this best plan
    const baseRelSizes2 = new Map();
    const maps = buildIncoming(baseBest);
    const nodeById2 = new Map((baseBest.nodes || []).map(n => [n.id, n]));
    for (const n of (baseBest.nodes || [])) {
      if (n.op !== "join") continue;
      const inE = maps.get(n.id) || [];
      for (const e of inE) {
        const port = e.port || "in";
        if (port !== "left" && port !== "right") continue;
        const fromNode = nodeById2.get(e.from);
        if (fromNode && fromNode.op === "join") continue;
        const rel = leafRelIdForNode(baseBest, e.from);
        const rows = rowsBase.get(e.from) ?? 1000;
        if (!baseRelSizes2.has(rel)) baseRelSizes2.set(rel, rows);
        else baseRelSizes2.set(rel, Math.min(baseRelSizes2.get(rel), rows));
      }
    }

    const res2 = rewriteJoinTreeV3_8(baseBest, rb.id, baseRelSizes2, js, { ...(o.joinOrder || {}), topK: perRoot });
    if (!res2.ok) continue;

    variants.components.push({ rootJoinId: rb.id, dp: res2.dp });

    // candidates excluding best
    const alts = (res2.candidates || []).slice(1).map(c => ({
      rank: c.rank,
      reason: "JOIN_ORDER_TOPK_DP_COORD",
      rootJoinId: rb.id,
      estCost: c.estCost,
      estRows: c.estRows,
      chosenOrder: c.chosenOrder,
    ...
    for (const a of alts) {
      if (variants.candidates.length >= maxTotal) break;
      variants.candidates.push(a);
    }
    if (variants.candidates.length >= maxTotal) break;
  }

  if (variants.candidates.length === 0) variants = null;
} catch (_) {}
  }
